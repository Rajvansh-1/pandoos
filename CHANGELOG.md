# Changelog

All notable changes to Pandoos are documented here.
Format follows [Keep a Changelog](https://keepachangelog.com/en/1.0.0/).

---

## [3.1.3] - 2026-06-02 🐼 The Pandoos Magic Update

Welcome to the **Pandoos Magic Update**! Our pandas have been working overtime to bring you a smoother, more magical listening experience across all your devices.

### ✨ What's New!
- **Seamless Auto-Updates:** Pandoos now quietly downloads updates in the background! When a new version is ready, a beautiful in-app banner will gently let you know. No more manual downloads, just uninterrupted music! 🎵
- **Native macOS Magic:** Pandoos is now fully supported and optimized for macOS! We've added proper native support so Mac users can jump straight into the music without annoying security warnings.

### 🚑 Bug Fixes & Improvements
- **The Great Content Fix:** Did your Artist and Album pages suddenly go blank recently? Our panda backend had a little hiccup syncing your music data. We've completely upgraded our resilient music engine—all your albums, top songs, and "Fans Also Like" sections are back and loading faster than ever! 🚀
- **Performance Boosts:** Behind-the-scenes optimizations to make navigating between tracks and albums feel buttery smooth.

---

## [3.0.8] - 2026-06-01 🌟 Stability Update
- Pandoos officially launched cross-platform syncing across Web, Desktop, and Android!
- Introduced the new Panda mascot animations based on the music's mood.
